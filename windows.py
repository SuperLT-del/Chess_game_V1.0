...
# 判断棋子走之后是否会出现将军碰面的情况
def have_two_jiang(li,num):
    line_y = 0
    while line_y < 3:
        if chessboard_map[line_y][num] == "b_j":
            line_y1 = 7
            while line_y1 < 10:
                if chessboard_map[line_y1][num] == "r_j":
                    flag_line = line_y + 1
                    while flag_line < line_y1:
                        if flag_line != li and chessboard_map[flag_line][num] != "":
                            return False
                        flag_line += 1
                    return True
                line_y1 += 1
...
def move_ok(status_be,status_pa,step):
    ...
    if status_be[1] >= 3 and status_be[1] <= 5:
        if status_pa[1] != status_be[1] and have_two_jiang(status_be[0],status_be[1]):
            return 1
    # 判断是否被将军
        ...
        while y1 < 9:
            if judge_red_black((i1,y1)) == str1:
                ...
                while i < 10:
                    ...
                    i += 1
                move_map = [[False for c in range(9)]for r in range(10)]
            elif judge_red_black((i1,y1)) == str1:
                draw_point_map((i1,y1))
                i = 0
                while i < 10:
                    y = 0 
